...
//       lib/services/clustering/ClusteringService.ts
...
export class ClusteringService {
  ...
  async cluster(texts: string[], options?: ClusteringOptions): Promise<ClusteringResult> {
    ...
    const result: ClusteringResult = {
      clusters,
      noise: pickByIndex(cleanTexts, dbscanResult.noise),
  ...
  private selectRepresentative(texts: string[]): string {
    ...
    // 不能就地 sort：调用方把同一个数组存进 ClusterResult.texts，
    // 而 indices 与之按位对应，就地排序会打乱这个对应关系。
    return longest(withKeyword.length > 0 ? withKeyword : texts);
  }
