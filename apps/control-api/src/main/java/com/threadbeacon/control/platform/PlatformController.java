...
public class PlatformController {
    ...
    @GetMapping("/workflows") Map<String,Object> workflows(@RequestParam(required=false)String projectId){return Map.of("workflows",platform.listWorkflows(user.ownerId(),projectId));}
    @PostMapping("/workflows") ResponseEntity<Map<String,Object>> createWorkflow(@RequestBody Map<String,Object> body){return ResponseEntity.status(HttpStatus.CREATED).body(Map.of("workflow",platform.createWorkflow(user.ownerId(),body)));}
    @GetMapping("/workflows/{id}") Map<String,Object> workflow(@PathVariable String id){return Map.of("workflow",platform.workflow(user.ownerId(),id));}
    @PatchMapping("/workflows/{id}") ResponseEntity<Map<String,Object>> workflowAction(@PathVariable String id,@RequestBody Map<String,Object> body){var action=text(body.get("action"));if("publish".equals(action))return ResponseEntity.ok(Map.of("version",platform.publish(user.ownerId(),id)));if("run".equals(action))return ResponseEntity.status(HttpStatus.CREATED).body(platform.run(user.ownerId(),id));return ResponseEntity.ok(Map.of("workflow",platform.saveWorkflow(user.ownerId(),id,body)));}
    @GetMapping("/workflows/runs/{id}") Map<String,Object> run(@PathVariable String id){return platform.runDetails(user.ownerId(),id);}
